...
export default function useJoin() {
  ...
  const validate = () => {
    ...
    else if (form.userId.length < MIN_ID) e.userId = `아이디는 최소 ${MIN_ID}자 이상이어야 합니다.`;

    if (!form.password.trim()) e.password = "비밀번호를 입력해 주세요.";
    else if (form.password.length < MIN_PW) e.password = `비밀번호는 최소 ${MIN_PW}자 이상이어야 합니다.`;

    if (!form.passwordConfirm.trim()) e.passwordConfirm = "비밀번호 확인을 입력해 주세요.";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setErrors((prev) => ({ ...prev, userId: "이미 존재하는 아이디이거나 서버 오류입니다." }));
    } finally {
      setLoading(false);
    }
  };
